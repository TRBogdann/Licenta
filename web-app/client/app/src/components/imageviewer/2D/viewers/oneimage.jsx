...
function OneImageView(props)
{
    ...
    useEffect(()=>
    {
        ...
        if(commandBox.command==='reset-index')
        {
            setImgIndex(0);
        ...
        img.onload = ()=>
        {
            ...
            {
                const backup = getItemWithId(content.effects,imgIndex); 
            ...
            setCopy(ctx.getImageData(0,0,img.width,img.height));

            if(palette.cmap!=='normal')
            {
                applyMap(canvas,palette.cmap);
            }
            if(palette.correction!=='none')
            {
                applyCorrection(canvas,content.ranges[imgIndex].a,content.ranges[imgIndex].b,palette.correction)
            }
        }

        img.src = content.images[imgIndex];

    },[content.images,imgIndex,palette])
        ...
        useEffect(() => {
            ...
            const handleMouseUp = () => {
                ...
                if (start && end) {
                    ...
                    const effVector = content.effects
                    const setEffects = content.setEffects
                    createBackup(
                        effVector,
                        setEffects,
                        {
                    ...
                    updateItem(content.images,content.setImages,imgIndex,newImg.image)               }
            };
            ...
            if(commandBox.command === 'clear-zoom')
                {
                    ...
                    const background = clearZoom(
                        ...
                        deleteBackup(content.imgBackup,content.setImgBackup,imgIndex)
                        updateItem(content.images,content.setImages,imgIndex,background)
                    }
