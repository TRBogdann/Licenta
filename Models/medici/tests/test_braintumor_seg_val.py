...
input_path = '../dataIN/MICCAI_BraTS2020_TestData/BraTS20_Validation_006/BraTS20_Validation_006_flair.nii'
...
for input_slice in input_nii:
    # Expand dimensions to add the channel dimension
    keep = input_slice
    input_slice = np.expand_dims(input_slice, axis=-1)  # Shape: (240, 155, 1)
    
    # Add a batch dimension
    input_slice = np.expand_dims(input_slice, axis=0)   # Shape: (1, 240, 155, 1)
    
    # Convert to TensorFlow tensor
    tensor = tf.convert_to_tensor(input_slice, dtype=tf.float32)
    print(tensor.shape)
    # Predict
